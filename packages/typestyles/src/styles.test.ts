...
describe('compose', () => {
  ...
  it('composes string class names', () => {
    const composed = compose('class-a', 'class-b', 'class-c');
    expect(composed()).toBe('class-a class-b class-c');
  });

  it('filters falsy values', () => {
    const composed = compose('class-a', false, null, undefined, 'class-b');
    expect(composed()).toBe('class-a class-b');
  });
});
...
describe('createStylesWithUtils', () => {
  ...
  it('expands utility keys in component() flat form', () => {
    const u = createStylesWithUtils({
      paddingY: (value: string | number) => ({ paddingTop: value, paddingBottom: value }),
    });

    u.component('util-comp-flat', {
      base: { paddingY: 12 },
    });
    ...
    const style = document.getElementById('typestyles') as HTMLStyleElement;
    const rule = Array.from(style.sheet?.cssRules ?? []).find(
      (r) => r instanceof CSSStyleRule && r.selectorText === '.util-comp-flat',
    ) as CSSStyleRule;
  ...
  it('expands utility keys in component() dimensioned form', () => {
    const u = createStylesWithUtils({
      marginX: (value: string | number) => ({ marginLeft: value, marginRight: value }),
    });

    const card = u.component('util-comp-dim', {
      base: { marginX: 10 },
      variants: { size: { sm: { marginX: 4 }, lg: { marginX: 20 } } },
      defaultVariants: { size: 'sm' },
    });

    expect(card()).toBe('util-comp-dim util-comp-dim-size-sm');
    expect(card({ size: 'lg' })).toBe('util-comp-dim util-comp-dim-size-lg');

    flushSync();

    const style = document.getElementById('typestyles') as HTMLStyleElement;
    const baseRule = Array.from(style.sheet?.cssRules ?? []).find(
      (r) => r instanceof CSSStyleRule && r.selectorText === '.util-comp-dim',
    ) as CSSStyleRule;
    const smRule = Array.from(style.sheet?.cssRules ?? []).find(
      (r) => r instanceof CSSStyleRule && r.selectorText === '.util-comp-dim-size-sm',
    ) as CSSStyleRule;

    expect(baseRule.style.getPropertyValue('margin-left')).toBe('10px');
    expect(smRule.style.getPropertyValue('margin-left')).toBe('4px');
  });
