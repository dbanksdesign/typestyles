...
describe('compose', () => {
  ...
  it('composes multiple component functions', () => {
    const base = createComponent('base', {
      base: { padding: '8px' },
    });
    const primary = createComponent('primary', {
      base: { color: 'blue' },
    });
  ...
  it('composes component functions with strings', () => {
    const baseStyles = createComponent('cbase', {
      base: { padding: '8px' },
    });
    const composed = compose(baseStyles, 'custom-class');

    expect(composed()).toBe('cbase custom-class');
  });

  it('filters falsy values', () => {
    const baseStyles = createComponent('base2', {
      base: { padding: '8px' },
    });
    const composed = compose(baseStyles, false, null, undefined, 'valid');

    expect(composed()).toBe('base2 valid');
  });

  it('returns empty string when all selectors return empty', () => {
    const a = createComponent('compose-a', {
      variants: { x: { val: { color: 'red' } } },
    });
    const b = createComponent('compose-b', {
      variants: { y: { val: { color: 'blue' } } },
    });
  ...
  it('handles mixed composition with component functions', () => {
    const btn = createComponent('base3', {
      base: { padding: '8px' },
      variants: {
        active: { true: { backgroundColor: 'blue' } },
      },
    });

    const composed = compose(btn, 'extra-class');

    expect(composed({ active: true })).toBe('base3 base3-active-true extra-class');
  });
});
...
describe('createStylesWithUtils', () => {
  ...
  it('expands utility keys in component()', () => {
    const u = createStylesWithUtils({
      paddingY: (value: string | number) => ({ paddingTop: value, paddingBottom: value }),
    });

    const btn = u.component('util-component', {
      base: { paddingY: 12 },
    });

    expect(btn()).toBe('util-component');
    flushSync();

    const style = document.getElementById('typestyles') as HTMLStyleElement;
    const rule = Array.from(style.sheet?.cssRules ?? []).find(
      (r) => r instanceof CSSStyleRule && r.selectorText === '.util-component',
    ) as CSSStyleRule;
