...
│   ├── styles.ts             # styles.component() — style registration and selector functions
...
```
Input:  styles.component('button', { base: { color: 'red' }, large: { fontSize: '18px' } })
...
If `styles.component()` is called with the same namespace twice, it warns in development and appends a suffix to avoid collisions. The developer should fix this — human-readable names only work if they're unique.
...
```
1. Module loads → styles.component('card', { root: { ... } }) called
