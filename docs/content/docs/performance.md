...
| ------------------------------------ | -------------- | ------------------------- |
| `styles.component()`                    | sub-ms         | Once per style definition |
| Selector function (`button()`) | very small     | Every render              |
...
// Usage: button({ primary: true, small: true })
```
...
- [ ] No `styles.component()` calls inside components
